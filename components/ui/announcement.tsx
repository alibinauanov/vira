...
function AnnouncementComponent({
  ...
  const displayContent = shiny
    ? React.Children.map(mainContentRef.current, (child) => (typeof child === 'string' ? <LustreText text={child} /> : child))
    ...
    >
      <div className="relative flex items-center gap-2">
        ...
        {hasExpandable ? (
          ...
          >
            <ChevronDown className="size-3 shrink-0 transition-transform duration-300 data-[state=open]:rotate-180" aria-hidden="true" data-state={isOpen ? 'open' : 'closed'} />
          </button>
        ) : null}
      </div>
    </Badge>
  ...
  return (
    <>
      <motion.div ref={containerRef} data-state={isOpen ? 'open' : 'closed'} {...animations[animation]} transition={{ duration: 0.3, ease: 'easeOut' }} className="relative inline-block">
        {movingBorder ? (
          <div className="relative overflow-hidden rounded-full bg-transparent p-[1px]">
            <div className="absolute inset-0 pointer-events-none">
              <MovingBorder duration={movingBorderDuration} rx="50%" ry="50%">
                <div className={cn('h-20 w-20 bg-[radial-gradient(#0ea5e9_40%,transparent_60%)] opacity-[0.8]', movingBorderClassName)} />
              </MovingBorder>
            </div>
            {badgeContent}
          </div>
...
export function AnnouncementTag({
  ...
  const tagContent = (
    <span className={cn('relative inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold rounded-full bg-background', className)} {...props}>
      <span className="absolute inset-0 rounded-full bg-foreground/5 opacity-70 pointer-events-none" />
      <span className="relative z-10">
        {React.Children.map(children, (child) => (lustre && typeof child === 'string' ? <LustreText text={child} /> : child))}
      </span>
    </span>
  );
    ...
    return (
      <span className="relative inline-block overflow-hidden rounded-full p-[1px]">
        <span className="absolute inset-0 pointer-events-none">
          <MovingBorder duration={movingBorderDuration} rx="50%" ry="50%">
            <div className={cn('h-12 w-12 bg-[radial-gradient(#0ea5e9_40%,transparent_60%)] opacity-80', movingBorderClassName)} />
          </MovingBorder>
        </span>
...
export function AnnouncementTitle({ className, multiTags = false, lustre = false, children, ...props }: AnnouncementTitleProps) {
  return (
    <span className={cn('inline-flex items-center gap-1.5 py-1', multiTags ? 'flex-wrap' : 'truncate', className)} {...props}>
      {React.Children.map(children, (child) => (lustre && typeof child === 'string' ? <LustreText text={child} /> : child))}
    </span>
  );
}
...
export function AnnouncementContainer({ className, ...props }: AnnouncementContainerProps) {
